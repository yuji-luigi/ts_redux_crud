...
import { flattenSectionData } from "../../../sectionData";
...
const reduxdb: Reduxdb = flattenSectionData.reduce<Reduxdb>(
...
export const crudSlice = createSlice({
  ...
  reducers: {
    ...
    selectCrudDocument: (state, action: PayloadAction<SelectCrudPayload>) => {
      ...
      if (document) {
        ...
        state.reduxdb[entity].selectedDocument =
          state.reduxdb[entity].selectedDocument.find(
            (doc: AllModels) => doc._id === documentId
          ) || null;
      }
    },
    resetStatus: (state) => {
    ...
    setCrudDocuments: (state, action) => {
      const { entity, documents, totalDocuments, isChildrenTree } =
        action.payload;
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(updateCrudDocument.fulfilled, (state, action) => {
        ...
        state.status = "succeed";
        const updatedDocuments = state.reduxdb[entity].documentsArray.map(
          (document) => {
            // TODO: FIX ERROR
...
const useTotalDocumentsCount = (entity: Sections): number =>
  useAppSelector(
    (state) => state.crud.reduxdb?.[entity || ""]?.totalDocuments || 0
  );
